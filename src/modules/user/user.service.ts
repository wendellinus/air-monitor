...
export class UserService {
  ...
  async getUserList(page: number, pageSize: number): Promise<{ list: Array<{ id: number; username: string; email?: string }>; total: number; page: number; pageSize: number }> {
    const { list, total } = await this.repo.list(page, pageSize);
    return {
      list: list.map((u) => ({ id: u.id, username: u.username, email: u.email ?? undefined })),
      total,
      page,
      pageSize,
    };
  ...
  async searchUsers(keyword: string | undefined, page: number, pageSize: number): Promise<{ list: Array<{ id: number; username: string; email?: string }>; total: number; page: number; pageSize: number }> {
