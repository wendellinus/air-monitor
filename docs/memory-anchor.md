...
详细从零启动/全量 API 清单请看：`docs/beginner-guide.md`。
...
- `src/modules/<domain>`：业务模块（auth/user/city/air/notice/provider/ws）
- `src/infra`：基础设施（Prisma、Redis、QWeather client/provider）
- `src/shared`：横切能力（统一错误/响应、DTO/校验、env 解析）
- `prisma/`：数据库 schema + migrations
- `docs/`：文档（你正在看的这个 + 新手文档）
