...
export class QweatherClient {
  ...
  async get<T>(path: string, params?: Record<string, string | number>): Promise<T> {
    ...
    if (auth.kind === 'apiKey' && host.includes('.re.qweatherapi.com')) {
      throw new Error('QWEATHER_HOST looks like a JWT host (*.re.qweatherapi.com). For API key mode, use https://devapi.qweather.com.');
    }
    if (auth.kind === 'bearer' && host.includes('devapi.qweather.com')) {
      throw new Error('QWEATHER_HOST is devapi.qweather.com (API key host). For JWT mode, use your project host like https://<id>.re.qweatherapi.com.');
    }
