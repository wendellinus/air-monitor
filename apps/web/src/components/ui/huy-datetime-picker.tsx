...
export type DateTimePickerProps = {
  ...
  placeholder?: string;
  texts?: Partial<DateTimePickerTexts>;
};

export type DateTimePickerTexts = {
  pickDateTime: string;
  clearDate: string;
  done: string;
  timezone: string;
...
export function DateTimePicker({
  ...
  const formatLocale = useMemo(() => props.locale as DateFnsLocale | undefined, [props.locale]);

  const uiTexts = useMemo<DateTimePickerTexts>(
    () => ({
      pickDateTime: texts?.pickDateTime || 'Pick a date and time',
      clearDate: texts?.clearDate || 'Clear date',
      done: texts?.done || 'Done',
      timezone: texts?.timezone || 'Timezone',
    }),
    [texts]
  );

  const formatOptions = useMemo(() => {
    return formatLocale ? { locale: formatLocale } : undefined;
  }, [formatLocale]);

  const dislayFormat = useMemo(() => {
    if (!displayValue) return placeholder || uiTexts.pickDateTime;
    return format(
      displayValue,
      `${!hideTime ? 'MMM' : 'MMMM'} d, yyyy${!hideTime ? (use12HourFormat ? ' hh:mm:ss a' : ' HH:mm:ss') : ''}`,
      formatOptions
    );
  }, [displayValue, hideTime, placeholder, use12HourFormat, uiTexts.pickDateTime, formatOptions]);
...
function MonthYearPicker({
  ...
  const months = useMemo(() => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      months.push({
        value: i,
        label: format(new Date(0, i), 'MMM', locale ? { locale } : undefined),
        disabled,
      });
    }
    return months;
